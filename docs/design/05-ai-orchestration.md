...
Model IDs must never be hard-coded without a startup validation path. The app should fail fast on an invalid configured model and expose a clear operational error.
...
1. try the configured fallback model profile,
